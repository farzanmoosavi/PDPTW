...
def test(n_node):
    ...
    if n_node==21 or n_node==51 or n_node==101:
        node_ = np.loadtxt('./test_data/vrp{}_test_data.csv'.format(n_node-1), dtype=float, delimiter=',')
        demand_ = np.loadtxt('./test_data/vrp{}_demand.csv'.format(n_node-1), dtype=float, delimiter=',')
        capcity_ = np.loadtxt('./test_data/vrp{}_capcity.csv'.format(n_node-1), dtype=float, delimiter=',')
        batch_size=128
    else:
