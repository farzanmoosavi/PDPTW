...
class CustomDataset(Dataset):
    def __init__(self, n_nodes, depots, n_robots, n_drones, num_samples):
        self.data = [self.create_instance(n_nodes, depots, n_robots, n_drones) for _ in range(num_samples)]

    def create_instance(self, n_nodes, depots, n_robots, n_drones):
        node_data = np.random.uniform(0, 3, (n_nodes + depots, 2))
        num_requests = n_nodes // 2
        lambda_rate = num_requests
        time_frame = 2 * 60  # in minutes
        ...
        early_time = np.ceil(arrival_times)
        late_time = early_time + np.random.randint(25, 50, size=(num_requests, 1))
        time_window = np.vstack((np.zeros((depots, 1)), early_time, late_time))

        edges_d, edges_index_d, mask_adjacency_d = self.generate_adjacency_network(node_data, time_window, mode='drone')
        edges_r, edges_index_r, mask_adjacency_r = self.generate_adjacency_network(node_data, time_window, mode='robot')

        demand = np.random.randint(1, 10, size=(n_nodes // 2))
        demand = np.hstack((demand, -demand))
        demand = np.insert(demand, 0, np.zeros(depots))
        ...
        Battery = {
            'drones': torch.tensor(np.ones((n_drones)) * 6500),
            'robots': torch.tensor(np.ones((n_robots)) * 4500)
        }
        battery = torch.cat((Battery['drones'], Battery['robots']))
    ...
    def generate_adjacency_network(self, node_data, time_windows, mode):
        node_data_tensor = torch.tensor(node_data, dtype=torch.float32)
        time_windows_tensor = torch.tensor(time_windows, dtype=torch.float32)
        ...
        if mode == 'drone':
            mu = random.randrange(1, 3)
            rho = random.randrange(20, 40)
        elif mode == 'robot':
            mu = random.randrange(1, 2)
            rho = random.randrange(10, 30)
...
def create_data(n_nodes, depots, n_robots, n_drones, num_samples, batch_size=32, num_workers=0, use_cache=False,
    ...
    if use_cache:
        try:
            ...
            dataset = CustomDataset(n_nodes, depots, n_robots, n_drones, num_samples)
...
def reward1(time_window, tour_indices, edge_attr_d, edge_attr_r, time, charge, num_drones):
    ...
    for step in range(steps - 1):
        ...
        is_delivery = current_indices >= num_depots + num_pickup
        T_current = time[:, :, step]
        # T_current[:, num_drones:] = T_current[:, num_drones:] / 4
        # Calculate penalties
        # current_time_window = torch.gather(time_window, 1, current_indices)
        pickup_t_window = torch.gather(time_window, 1, current_indices) * is_pickup
        ...
        penalty_pickup = torch.where(is_consecutive_depot, torch.zeros_like(penalty_pickup), penalty_pickup)
        penalty_delivery = torch.where(is_consecutive_depot, torch.zeros_like(penalty_delivery), penalty_delivery)
        T_current[:, num_drones:], penalty_pickup[:, num_drones:], penalty_delivery[:, num_drones:] = T_current[:,
                                                                                                      num_drones:] / 2.5, penalty_pickup[
                                                                                                                          :,
                                                                                                                          num_drones:] / 2.5, penalty_delivery[
                                                                                                                                              :,
                                                                                                                                              num_drones:] / 2.5
        total_cost += (alpha * T_current/n_agent + penalty_pickup + penalty_delivery) / num_pickup
