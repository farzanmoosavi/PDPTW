...
def plot_vehicle_routes(data, route, ax1, Greedy, markersize=5, visualize_demands=False, demand_scale=1,
                        round_demand=False):
    ...
    for veh_number, r in enumerate(routes):
        ...
        qvs.append(qv)
    if Greedy:
        ax1.set_title('Greedy,{} routes, total distance {:.2f}'.format(len(routes), total_dist),
                      family='Times New Roman', size=20)
    else:
        ax1.set_title('Sampling1280,{} routes, total distance {:.2f}'.format(len(routes), total_dist),
                      family='Times New Roman', size=20)
...
def vrp_matplotlib(Greedy=True):
    node1 = np.loadtxt('./test_data/vrp20_test_data.csv', dtype=float, delimiter=',')
    demand1 = np.loadtxt('./test_data/vrp20_demand.csv', dtype=float, delimiter=',')
    capcity1 = np.loadtxt('./test_data/vrp20_capcity.csv', dtype=float, delimiter=',')
    node1, demand1 = node1.reshape(-1, n_nodes, 2), demand1.reshape(-1, n_nodes)
    data_size = node1.shape[0]
    ...
    for i, (x1, y1) in enumerate(node1[x]):
        for j, (x2, y2) in enumerate(node1[x]):
    ...
    datas = []
    data = Data(x=torch.from_numpy(node1[x]).float(), edge_index=edges_index,
                edge_attr=torch.from_numpy(edges1).float(),
                demand=torch.tensor(demand1[x]).unsqueeze(-1).float(),
                capcity=torch.tensor(capcity1[x]).unsqueeze(-1).float())
    datas.append(data)
